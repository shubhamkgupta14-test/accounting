...
export default function AccountSummary() {
  ...
  return <div>
    <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between' }}>
      ...
      <ExportMenu fullReport title="Account Summary" rows={accounts.map(a => ({ code: a.code, account: a.name, type: a.type, group: a.group, balance: a.balance || 0 }))} />
    </div>
    <div className="card"><table className="data-table"><thead><tr><th>Type / Account</th><th>Group</th><th className="num total-amount">Balance ({currencySymbol})</th></tr></thead>
      <tbody>{Object.entries(groups).map(([type, rows]) => <Fragment key={type}>
        ...
        {rows.map(a => <tr key={a.id}><td style={{ paddingLeft: 32 }}>{a.name}</td><td><span className="group-text">{a.group}</span></td><td className={`num ${['Asset', 'Expense'].includes(a.type) ? 'dr-amount' : 'cr-amount'}`}>{Math.abs(a.balance || 0).toLocaleString('en-IN')}</td></tr>)}
      </Fragment>)}</tbody>
