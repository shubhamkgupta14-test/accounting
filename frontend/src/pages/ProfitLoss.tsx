        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr' }}>
          <div style={{ borderRight: '1px solid #E2E8F0' }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}><tbody>
              ...
              {expenses.map(account => <tr key={account.id} style={{ borderBottom: '1px solid #F1F5F9' }}><td style={{ padding: '8px 20px', fontSize: 13 }}><span style={{ display: 'flex', alignItems: 'center', gap: 9 }}><AuditCheckbox item={account.name} />{account.name}</span></td><td style={{ padding: '8px 20px', textAlign: 'right', fontFamily: 'JetBrains Mono, monospace' }}>{(account.balance || 0).toLocaleString('en-IN')}</td></tr>)}
